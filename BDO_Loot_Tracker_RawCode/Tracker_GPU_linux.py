# ============================================================
# GPU ONNX + DXGI OPTIMIZED TRACKER
#
# [LINUX REQUIREMENTS]
# pip install mss pynput onnxruntime-gpu customtkinter pillow numpy opencv-python rapidocr_onnxruntime requests pygame
# For X11 support on Linux: sudo apt-get install python3-tk python3-dev libx11-dev libxtst-dev
#
# [WINDOWS REQUIREMENTS]
# pip install dxcam onnxruntime-gpu
#
# Optional NVIDIA:
# pip install onnxruntime-gpu
#
# Optional AMD/Intel DirectML:
...
class LootTrackerApp(ctk.CTk):
    def __init__(self):
        ...
        self.current_hotkey_code = VK_MAP.get(self.current_hotkey_str, 0x79 if IS_WINDOWS else keyboard.Key.f10)
    ...
    def init_window_styles(self):
        try:
            if IS_WINDOWS:
                ...
                # Linux: Use tkinter attributes for window management
                self.attributes("-topmost", True)
                self.hwnd = None  # Not used on Linux
        except Exception as e:
    ...
    def hotkey_check_loop(self):
        if IS_WINDOWS:
            ...
            # Linux: Use pynput for hotkey detection
            def on_press(key):
                if key == self.current_hotkey_code:
                    self.update_queue.put(("toggle_click", None))
    ...
    def update_loop(self):
        ...
        if IS_WINDOWS:
            ...
            # Linux: Use mss for screen capture
            camera = mss.mss()
            region = {
                "left": self.scan_region["left"],
                "top": self.scan_region["top"],
                "width": self.scan_region["width"],
                "height": self.scan_region["height"]
            }
        ...
        try:
            while self.session_running:
                try:
                    ...
                    if not bypass_filters and frame_count % 3 != 0:
            ...
            # Release camera (platform-specific)
            if camera:
                if IS_WINDOWS:
                    camera.release()
                else:
        ...
        try:
            ...
            if self.is_click_through:
                ...
                if IS_WINDOWS:
                    ...
                    # Linux: Limited click-through support via window attributes
